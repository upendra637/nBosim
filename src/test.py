...
moon_a_display = 0.35            # AU -- exaggerated, purely for visibility

time_years = np.linspace(0, simulation_years, frames)
moon_angle = 2 * np.pi * time_years / moon_period_years

moon_relative_x = moon_a_display * np.cos(moon_angle)
moon_relative_y = moon_a_display * np.sin(moon_angle)
...
# Save alongside the project files, regardless of where the command is run.
output_file = Path(__file__).resolve().parent.parent / "solar_system.gif"
animation.save(output_file, writer="pillow", fps=30)
